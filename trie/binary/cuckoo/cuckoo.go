...
// Filter 是一个紧凑的布谷鸟过滤器。
type Filter struct {
	buckets [numBuckets][slotsPerBucket]uint16
	count   int
}
...
// hash 返回数据的原始哈希值和指纹。
func hash(data []byte) (uint, uint16) {
	h := crypto.Keccak256(data)
	// 使用前 4 个字节计算桶索引
	i1 := uint(binary.BigEndian.Uint32(h[0:4])) & (numBuckets - 1)
	// 使用接下来的 2 个字节作为指纹
...
func alternateIndex(i uint, fp uint16) uint {
	fpBytes := make([]byte, 2)
	...
	return (i ^ uint(binary.BigEndian.Uint32(h[0:4]))) & (numBuckets - 1)
}
...
// 格式如下：
// - 16 字节：位图（128 位，每个位代表一个槽位，1=已占用，0=空闲）
...
func (f *Filter) Encode() []byte {
	...
	res := make([]byte, 16+len(fingerprints))
	copy(res[0:16], bitmask[:])
	copy(res[16:], fingerprints)
	return res
}
	...
	for i := 0; i < numBuckets; i++ {
		for s := 0; s < slotsPerBucket; s++ {
			...
			if (bitmask[slotIdx/8] & (1 << (7 - (slotIdx % 8)))) != 0 {
				...
				f.buckets[i][s] = binary.BigEndian.Uint16(fingerprints[fpIdx : fpIdx+2])
				f.count++
				fpIdx += 2
			}
		}
